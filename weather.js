...
getWeatherInfo = (place) => {
    ...
    maxTemp = doc.select(".up_temperature>strong").text();
    minTemp = doc.select(".down_temperature>strong").text();
	...
	dailyData = doc.select(".daily_weather_list").select("li").get(i); 
      dailyResponse += "\n\n▷  " + String(dailyData.select(".date").text());
